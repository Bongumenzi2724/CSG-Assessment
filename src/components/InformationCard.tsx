...
const InformationCard: React.FC<ItemsProps> = ({item}:ItemsProps) => {
  
  const imageStr='Images/CommunionThumb.jpg'
  mainURL=mainURL.replace(/^"|"$/g,'');
  //let imageURL=item.Image.replace(/^"|"$/g,'');
  let imageURL=item.Image!=null?item.Image.replace(/^"|"$/g,''):imageStr.replace(/^"|"$/g,'');
  imageURL=`${mainURL}/${imageURL}`;

  //let audioURL=item.Audio.replace(/^"|"$/g,'');
  
  const audioStr='MP3/XmPqugb4EqQ.mp3'
  let audioURL=item.Audio!=null?item.Audio.replace(/^"|"$/g,''):audioStr.replace(/^"|"$/g,'');
    ...
    <div className="max-w-sm rounded relative w-full">
      ...
      <div className="py-4 flex flex-col gap-3">
        ...
        <div className="flex flex-col justify-center items-start gap-2">
          <h3 className='text-base font-bold text-[#163d69]'>Category: {item.Category}</h3>
          <p className="text-base text-[#163d69] font-bold">
            Create Date : {item.CreateDate}
          </p>
        </div>
        <div className="flex flex-row gap-10 items-center">
          <p className="text-base font-bold text-[#326084]">Episode: {item.Episode}</p>
          <p className="text-base font-bold text-[#326084]">MediaName: {item.MediaName}</p>
        </div>
      </div>
    </div>
  )
